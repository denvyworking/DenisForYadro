...
func Load(path string) (Source, error) {
	sources, err := LoadAll(path)
	if err != nil {
		return Source{}, err
	}
	if len(sources) == 0 {
		return Source{}, fmt.Errorf("archive has no files")
	}
	return sources[0], nil
}

func LoadAll(path string) ([]Source, error) {
	...
	case strings.HasSuffix(lower, ".zip"):
		return readZipAll(source)
	case strings.HasSuffix(lower, ".tar.gz") || strings.HasSuffix(lower, ".tgz"):
		return readTarGzAll(source)
	case strings.HasSuffix(lower, ".gz"):
		return readGzipAll(source)
	default:
		return []Source{source}, nil
	}
}

func readZipAll(source Source) ([]Source, error) {
	...
	for _, file := range zipReader.File {
		...
		files = append(files, Source{Path: source.Path + "::" + file.Name, Data: content, Size: int64(len(content)), ModTime: file.Modified})
	}
	if len(files) == 0 {
...
func readTarGzAll(source Source) ([]Source, error) {
	gzReader, err := gzip.NewReader(bytes.NewReader(source.Data))
	if err != nil {
		return nil, fmt.Errorf("open gzip: %w", err)
	}
	defer gzReader.Close()
	return readTarAll(source.Path, gzReader)
}

func readTarAll(path string, reader io.Reader) ([]Source, error) {
	...
	for {
		...
		files = append(files, Source{Path: path + "::" + hdr.Name, Data: content, Size: int64(len(content)), ModTime: hdr.ModTime})
	}
	if len(files) == 0 {
...
func readGzipAll(source Source) ([]Source, error) {
	gzReader, err := gzip.NewReader(bytes.NewReader(source.Data))
	if err != nil {
		return nil, fmt.Errorf("open gzip: %w", err)
	}
...
func readTar(path string, reader io.Reader) (Source, error) {
	files, err := readTarAll(path, reader)
	if err != nil {
		return Source{}, err
	}
	return files[0], nil
